    ...
    const fetchBatchPrograms = async () => {
      ...
      try {
        ...
        if (
          !batchData ||
          !batchData.programs ||
          !Array.isArray(batchData.programs)
        ) {
        ...
        {isOpen && (
          <View style={styles.filterList}>
            {programs.length > 0 ? (
              ...
              <View style={styles.filterEmpty}>
                <Text style={styles.filterEmptyText}>
                  No programs available for this batch
                </Text>
              </View>
            )}
          </View>
...
const InvalidStudentDialog = memo(
  ({
    isVisible,
    onClose,
    studentName,
    studentId,
  }: InvalidStudentDialogProps) => {
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.confirmationDialog}>
              ...
              <Text style={styles.confirmationMessage}>
                {studentName} ({studentId}) is not enrolled in the selected
                batch and program.
              </Text>
              <TouchableOpacity
                style={[styles.confirmationButton, styles.confirmButton]}
...
export default function AttendanceScreen() {
  ...
  const [invalidStudent, setInvalidStudent] = useState<{
    name: string;
    id: string;
  } | null>(null);
  ...
  const fetchInitialData = async () => {
    try {
      await Promise.all([fetchBatches(), fetchPrograms(), fetchStudents()]);
    } catch (error) {
  ...
  const filteredStudents = students.filter((student) => {
    ...
    const matchesBatch =
      !selectedBatch || student.batch?.id === selectedBatch.id;
    const matchesProgram =
      !selectedProgram ||
      student.programs.some((p) => p.id === selectedProgram.id);
      ...
      >
        <View>
          ...
          <View style={styles.filters}>
            <BatchSelector value={selectedBatch} onSelect={setSelectedBatch} />
            <ProgramSelector
          ...
          <View style={styles.bulkActions}>
            ...
            >
              <MaterialCommunityIcons
                ...
                color={COLORS.white}
              />
              <Text style={styles.bulkActionText} bold>
                Mark All Present
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
                  ...
                  <View style={styles.attendanceStatus}>
                    {record?.markedBy === "scan" && (
                      <MaterialCommunityIcons
                        ...
                        size={20}
                        color={
                          !isSelectionValid ? COLORS.gray : COLORS.textLight
                        }
